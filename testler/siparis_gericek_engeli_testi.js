...
t('startEditSiparis girişte sevk_edildi\'yi reddediyor',
  /function startEditSiparis\(main, siparis\) \{/.test(src) &&
  /if \(siparis\.durum === 'sevk_edildi'\) \{[\s\S]{0,200}return;\s*\}/.test(src));
t('"Geri Çek / Düzenle" butonu sevk_edildi\'de RENDER EDİLMİYOR',
  /\$\{s\.durum !== 'sevk_edildi' \? '<button class="btn btn-amber" id="sp-geri-cek">/.test(src));
...
t('kayıt anında TAZE veriyle tekrar kontrol ediliyor (mevcut.durum)',
  /if \(mevcut\.durum === 'sevk_edildi'\) \{[\s\S]{0,150}return;/.test(src));
