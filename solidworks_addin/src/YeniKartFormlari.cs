            ...
            tamamBtn.Click += (s, e) =>
            {
                ...
                try
                {
                    ...
                    else if (_kartTipi == "yarimamul")
                    {
                        ...
                        kart["kod"] = kod;
                        kart["ad"] = ad;
                        kart["netBoy"] = string.IsNullOrWhiteSpace(netBoyKutusu.Text) ? null : (object)Cift(netBoyKutusu);
                        kart["netEn"] = string.IsNullOrWhiteSpace(netEnKutusu.Text) ? null : (object)Cift(netEnKutusu);
                        kart["kalinlik"] = string.IsNullOrWhiteSpace(ymKalinlikKutusu.Text) ? null : (object)Cift(ymKalinlikKutusu);
                        kart["kabaBoy"] = string.IsNullOrWhiteSpace(kabaBoyKutusu.Text) ? null : (object)Cift(kabaBoyKutusu);
                        kart["kabaEn"] = string.IsNullOrWhiteSpace(kabaEnKutusu.Text) ? null : (object)Cift(kabaEnKutusu);
                        kart["adet"] = Tam(adetKutusu, 1);
                        kart["renk"] = (ymRenkKutusu.Text ?? "").Trim();
                        kart["referansFiyat"] = string.IsNullOrWhiteSpace(referansFiyatKutusu.Text) ? null : (object)Cift(referansFiyatKutusu);
